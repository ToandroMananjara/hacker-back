...
class ProfilePhotoController
{
    ...
    public function createCoverPhoto()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
            http_response_code(200);
            exit;
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (isset($_FILES['coverPhoto']) && $_FILES['coverPhoto']['error'] === UPLOAD_ERR_OK) {
                    ...
                    $uploadDir = 'users/' . $userId . '/coverPhoto';
                    ...
                    $coverPhoto = new CoverPhoto($conn);
                    $coverPhoto->user_id = $userId;
                    $coverPhoto->photo_path = $destination;

                    // Enregistrer le chemin dans la base de données (assurez-vous que la méthode insert() est définie dans la classe coverPhoto)
                    if (!$coverPhoto->create()) {
                        throw new Exception("Une erreur est survenue lors de l'enregistrement du chemin dans la base de données.");
                    }

                    // Répondre avec un message de succès
                    echo json_encode(['status' => 'success', 'message' => 'Photo de couverture téléchargée avec succès.', 'photo_path' => $destination], JSON_UNESCAPED_UNICODE);
                } else {
